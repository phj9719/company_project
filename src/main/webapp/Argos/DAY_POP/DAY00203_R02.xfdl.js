(function()
{
    return function()
    {
        ...
        {
            this.set_name("DAY00203_R01");
            this.set_titletext("간호기구소독일지(7일)");
            if (Form == this.constructor)
            {
                this._setFormPosition(1300,900);
            }
            ...
            // Layout Functions
            //-- Default Layout : this
            obj = new Layout("default","",1300,900,this,function(p){});
        ...
        // User Script
        this.registerScript("DAY00203_R02.xfdl", function() {
        /************************************************************************************
        * 뷰어 초기 설정
        ************************************************************************************/
        ...
        ************************************************************************************/
        ...
        {
        	let jrfFile = "DAY00203_R02.jrf";

        	// UbiReport Argument 세팅
        	const sicode = this.parent.pParam.sicode;
        	const dDate = this.parent.pParam.d_date;
        	const maxDate = this.parent.pParam.max_date;
        	const minDate = this.parent.pParam.min_date;
        	const sign = this.parent.pParam.sign;

        	this.UbiViewer.ismultireport = "false";

        	this.UbiViewer.setArgument("P_SICODE", sicode);
        	this.UbiViewer.setArgument("P_MAXDATE", maxDate);
        	this.UbiViewer.setArgument("P_MINDATE", minDate);
        	this.UbiViewer.setArgument("P_DATE", dDate);
        ...
        ************************************************************************************/
        ...
        this.loadIncludeScript("DAY00203_R02.xfdl");
