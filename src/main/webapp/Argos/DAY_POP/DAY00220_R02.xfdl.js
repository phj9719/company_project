(function()
{
    return function()
    {
        ...
        {
            this.set_name("DAY00220_R02");
            this.set_titletext("수급자 주간투약전달일지");
        ...
        // User Script
        this.registerScript("DAY00220_R02.xfdl", function() {
        /************************************************************************************
        * 뷰어 초기 설정
        ************************************************************************************/
        ...
        ************************************************************************************/
        ...
        {
        	let jrfFile = "DAY00220_R02.jrf";

        	// UbiReport Argument 세팅
        	const sicode = this.parent.pParam.sicode;
        	const dateFrom = this.parent.pParam.dateFrom;
        	const dateTo = this.parent.pParam.dateTo;
        	const sign = this.parent.pParam.sign;
        	const mCode = this.parent.pParam.mCode;

        	this.UbiViewer.ismultireport = "false";

        	this.UbiViewer.setArgument("P_SICODE", sicode);
        	this.UbiViewer.setArgument("P_M_CODE", mCode);
        	this.UbiViewer.setArgument("P_SIGN", sign);
        	this.UbiViewer.setArgument("P_DATE_FROM", dateFrom.toISOString().substr(0,10));
        	this.UbiViewer.setArgument("P_DATE_TO", dateTo.toISOString().substr(0,10));
        ...
        ************************************************************************************/
        ...
        /*
        	...
        	this.UbiViewer.multicount = ""+mCodeArray.length+"";
        	...
        	for (let i = 0; i < mCodeArray.length; i++){
        		if(i ==  mCodeArray.length-1){
        			sicodeArr += "P_SICODE."+(i+1)+"#"+sicode;
        			dateFromArr += "P_DATE_FROM."+(i+1)+"#"+dateFrom.toISOString().substr(0,10);
        			dateToArr += "P_DATE_TO."+(i+1)+"#"+dateTo.toISOString().substr(0,10);
        			signArr += "P_SIGN."+(i+1)+"#"+sign;
        			mCodeArrayArr += "P_M_CODE."+(i+1)+"#"+ mCodeArray[i];
        		}else{
        			sicodeArr += "P_SICODE."+(i+1)+"#"+sicode+"#";
        			dateFromArr += "P_DATE_FROM."+(i+1)+"#"+dateFrom.toISOString().substr(0,10)+"#";
        			dateToArr += "P_DATE_TO."+(i+1)+"#"+dateTo.toISOString().substr(0,10)+"#";
        	...
        	this.UbiViewer.arg = sicodeArr+"#"+dateFromArr+"#"+dateToArr+"#"+signArr+"#"+mCodeArrayArr;
        	*/
        });
        ...
        this.loadIncludeScript("DAY00220_R02.xfdl");
