(function()
{
    return function()
    {
        ...
        {
            this.set_name("DAY00102_5_R02");
            this.set_titletext("상태변화기록");
        ...
        // User Script
        this.registerScript("DAY00102_5_R02.xfdl", function() {
        /************************************************************************************
        * 뷰어 초기 설정
        ************************************************************************************/
        ...
        ************************************************************************************/
        ...
        this.form_onload = function(obj,e)
        {
        	...
        	const m_code = this.parent.pParam.m_code;
        	const fdate = this.parent.pParam.fdate;
        	const tdate = this.parent.pParam.tdate;
        	const sign = this.parent.pParam.sign;
        	const flag = this.parent.pParam.flag;
        	const size = m_code.length; //배열 길이
        	//flag로 multireport 구분
        	if (flag != "1" )
        	{
        	...
        	{//multireport
        		...
        		this.UbiViewer.multicount = size;
        		for(let i=0;i<size;i++){
        			this.UbiViewer.setArgument("P_M_CODE." + (i+1), m_code[i]);
        		}// P_M_CODE.1 : m_code[1] , P_M_CODE.2 : m_code[2]...
        	}
        	//UbiDesigner의 arguments에 값 세팅
        	this.UbiViewer.setArgument("P_SICODE", sicode);
        	this.UbiViewer.setArgument("P_DATE_FROM", fdate);
        	this.UbiViewer.setArgument("P_DATE_TO", tdate);
        	this.UbiViewer.setArgument("P_SIGN", sign);

        	// 리포트 파일명
            this.UbiViewer.jrffile = "DAY00102_5_R02.jrf";
        ...
        ************************************************************************************/
        ...
        this.loadIncludeScript("DAY00102_5_R02.xfdl");
