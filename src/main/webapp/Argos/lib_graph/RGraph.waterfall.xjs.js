...
(function()
{
    return function(path)
    {
        ...
        if (!nexacro.RGraph.Waterfall)
        {
        	 ...
        	 */
        	nexacro.RGraph.Waterfall = function (conf)
        	{
        		var RG = arguments[1];
        		...
        		{
        			var context = co,numYLabels = 5,interval = this.grapharea / numYLabels,font = prop["chart.text.font"],size = prop["chart.text.size"],color = prop["chart.text.color"],units_pre = prop["chart.units.pre"],units_post = prop["chart.units.post"],offsetx = prop["chart.ylabels.offsetx"],offsety = prop["chart.ylabels.offsety"];
        			...
        			this.Drawbars = function ()
        		{
        			...
        			for (var i = 0,len = this.data.length; i < len; ++i)
        			{
        				...
        				// Color
        				co.fillStyle = colcol = this.data[i] >= 0 ? prop["chart.colors"][0] : prop["chart.colors"][1];
        				RG.setElementCommonStyle(co, co.fillStyle, 1);

        				co.rect(x, Math.floor(y), w, Math.floor(h));
        			...
        			if (prop["chart.total"])
        			{
        				...
        				// Final color
        				co.fillStyle = colcol = prop["chart.colors"][2];
        				RG.setElementCommonStyle(co, co.fillStyle, 1);
        		...
        		this.grow = function ()
        		{
        			...
        			function iterator ()
        			{
        				for (var i = 0; i < obj.data.length; ++i)
        				{
        					...
        					obj.data[i] = data[i] * RG.Effects_getEasingMultiplier(frames, numFrame);
        				}
