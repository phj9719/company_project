...
(function()
{
    return function(path)
    {
        ...
        if (!nexacro.RGraph.Rscatter)
        {
        	 ...
        	 * @param array data
        	 */
        	nexacro.RGraph.RScatter =
        		nexacro.RGraph.Rscatter = function (conf)
        	{
        		var RG = arguments[1];
        		...
        		this.properties =
        			...
        			'chart.labels.axes.background' : '#ffffffaa',
        			...
        			'chart.key.interactive.highlight.chart.fill' : '#ff000099',
        			...
        			'chart.segment.highlight.fill' : [0, 255, 0, 0.5],
        			'chart.segment.highlight.stroke' : [0, 0, 0, 0],
        			'chart.line' : false,
        			...
        			this.Set = function (name, value)
        		{
        			...
        			if (name === 'chart.segments.highlight.count')
        			{
        				name = 'chart.segment.highlight.count';
        			}
        			...
        			this.DrawRscatter = function ()
        		{
        			for (var dataset = 0; dataset < this.data.length; dataset += 1)
        			{
        				...
        				var drawPoints = function (obj)
        				{
        					...
        					{
        						var d1 = data[i][0],d2 = data[i][1],a = d1 / (180 / RG.PI),r = ((d2 - prop["chart.ymin"]) / (obj.scale2.max - obj.scale2.min)) * obj.radius,x = ma.sin(a) * r,y = ma.cos(a) * r,color = data[i][2] ? data[i][2] : prop["chart.colors.default"],tooltip = data[i][3] ? data[i][3] : null;
        		...
        		this.drawLine = function (idx)
        		{
        			...
        			if ((typeof prop["chart.line.close"] === 'boolean' && prop["chart.line.close"])
        		...
        		{
        			var r = r + 10,color = prop["chart.labels.color"];
