//XJS=RGraph.semicircularprogress.xjs
(function()
{
    return function(path)
    {
         ...
         * o--------------------------------------------------------------------------------o
         */
        if (!nexacro.RGraph.SemiCircularProgress)
        {
        	/**
        	 ...
        	 */
        	nexacro.RGraph.SemiCircularProgress = function (conf)
        	{
        		var RG = arguments[1];
        		...
        		this.properties =
        			...
        			'chart.background.color' : '#ffffffff',
        			'chart.colors' : ['#00cc00'],
        			'chart.linewidth' : 2,
        			'chart.strokestyle' : '#666666',
        		...
        		this.draw =
        			this.Draw = function ()
        		{
        			if (RG.ISRUNTIME || RG.ISIE8)
        			{
        				this.set("anglesStart", Math.PI);
        				this.set("anglesEnd", (2 * Math.PI));
        			}
        			...
        			if (typeof prop["chart.width"] === 'number')
        			{
        				this.width = prop["chart.width"];
        			}
        			...
        			this.DrawMeter = function ()
        		{
        			...
        			//
        			var start = prop["chart.angles.start"],end = prop["chart.angles.end"];
        			...
        			if (prop["chart.background.color"] !== '#00000000' && prop["chart.background.color"] !== '[0,0,0,0]' && prop["chart.background.color"] !== [0, 0, 0, 0])
        			{
        				RG.pa2(co, 'fs % fr % % % %',
        			...
        			RG.pa2(
        				co,
        				'lw % b a % % % % % false a % % % % % true c s % f % sx % sy % sc % sb % f % sx 0 sy 0 sb 0 sc #00000000 lw 1',
        				...
        				prop["chart.shadow.offsetx"], prop["chart.shadow.offsety"], prop["chart.shadow"] ? prop["chart.shadow.color"] : '#00000000', prop["chart.shadow.blur"],
        				typeof prop["chart.colors"][1] !== 'undefined' ? [0, 0, 0, 0] : [255, 255, 255, 0.85]
        			);
        			...
        			if (RG.ISRUNTIME)
        			{
        				...
        				co.beginPath();
        				co.arc(this.centerx, this.centery, this.radius, start, angle, false);
        				...
        				RG.setElementCommonStyle(co, co.fillStyle, 1);
        				co.beginPath();
        				co.arc(this.centerx, this.centery,
        					this.radius - this.width, 0, 2 * Math.PI, false);
        				co.lineTo(this.centerx, this.centery);
        				co.closePath();
        				co.fill();
        				co.beginPath();
        				co.arc(this.centerx, this.centery,
        					...
        					start + ((end - start) * ((this.value - this.scale2.min) / (this.max - this.scale2.min))), start, false);
        				co.closePath();
        				co.fill();
        			}
        			else
        			...
        			this.DrawLabels = function ()
        		{
        			...
        			if (prop["chart.angles.start"] === RG.PI)
        			{
        				var halign = 'center';
        				var valign = 'top';
        			}
        			else if (prop["chart.angles.start"] <= RG.PI)
        			{
        				var halign = 'left';
        				var valign = 'center';
        			}
        			...
        			// Determine the horizontal and vertical alignment for the text
        			if (prop["chart.angles.end"] === RG.TWOPI)
        			{
        				var halign = 'center';
        				var valign = 'top';
        			}
        			else if (prop["chart.angles.end"] >= RG.TWOPI)
        			{
        				var halign = 'right';
        				var valign = 'center';
        			}
        		...
        		{
        			var mouseXY = RG.getMouseXY(e),mouseX = mouseXY[0],mouseY = mouseXY[1],angle = RG.getAngleByXY(
        				this.centerx,
        				this.centery,
        				mouseX,
        				mouseY
        		...
        		this.grow = function ()
        		{
        			var obj = this,initial_value = this.currentValue,opt = arguments[0] || {
        			},numFrames = opt.frames || 30,frame = 0,callback = arguments[1] || function ()
        			{
        			},diff = this.value - Number(this.currentValue),increment = diff / numFrames;
