...
(function()
{
    return function(path)
    {
        ...
        if (!nexacro.RGraph.Scatter)
        {
        	 ...
        	 */
        	nexacro.RGraph.Scatter = function (conf)
        	{
        		var RG = arguments[1];
        		...
        		this.data = RG.arrayClone(conf.data);
        		...
        		{
        			var tmp = RG.arrayClone(conf.data);
        			conf.data = new Array();
        			conf.data[0] = RG.arrayClone(tmp);
        		...
        		for (var i = 0,len = this.data.length; i < len; ++i)
        		{
        			...
        			for (var j = 0,len2 = this.data[i].length; j < len2; ++j)
        			{
        				...
        				if (typeof this.data[i][j] === 'object' && !RG.isNull(this.data[i][j]) && typeof this.data[i][j][0] === 'string')
        				{
        				...
        				if (typeof this.data[i][j] === 'object' && !RG.isNull(this.data[i][j]) && typeof this.data[i][j][1] === 'string')
        				{
        		...
        		this.properties = {
        			...
        			'chart.background.barcolor1' : '#00000000',
        			'chart.background.barcolor2' : '#00000000',
        			...
        			'chart.line.colors' : ['green', 'red', 'blue'],
        			'chart.line.shadow.color' : '#00000000',
        			...
        			'chart.key.interactive.highlight.chart.fill' : [255, 0, 0, 0.9],
        		...
        		/**
        		 * This allows the data points to be given as dates as well as numbers. Formats supported by RG.parseDate() are accepted.
        		 *
        		...
        		for (var i = 0; i < this.data.length; ++i)
        		{
        			for (var j = 0; j < this.data[i].length; ++j)
        			{
        				...
        				{
        					this.data[i][j][0] = RG.parseDate(this.data[i][j][0]);
        				}
        			}
        		}
        			...
        			this.Draw = function ()
        		{
        			 ...
        			 */
        			if (typeof (prop["chart.xmin"]) == 'string')
        			{
        				prop["chart.xmin"] = RG.parseDate(prop["chart.xmin"]);
        			}
        			if (typeof (prop["chart.xmax"]) == 'string')
        			{
        				prop["chart.xmax"] = RG.parseDate(prop["chart.xmax"]);
        			}
        			...
        			/**
        			 * Draw the "in graph" labels, using the member function, NOT the shared function in RG.common.core.js
        			 */
        			...
        			this.DrawAxes = function ()
        		{
        			...
        			{
        				var x = 0,y = this.getYCoord(prop["chart.ylabels.invert"] ? this.scale2.max : (this.scale2.max > 0 && this.scale2.min > 0 ? this.scale2.min : 0)) - 3,size = 3;
        			...
        			this.DrawLabels = function ()
        		{
        			...
        			if (prop["chart.xscale"])
        			{
        				...
        				{
        					var num = ((prop["chart.xmax"] - prop["chart.xmin"]) * ((i + 1) / numXLabels)) + (xmin || 0),x = this.gutterLeft + ((i + 1) * interval),text = typeof prop["chart.xscale.formatter"] === 'function' ? String(prop["chart.xscale.formatter"](this, num)) : this.xscale2.labels[i];
        		...
        		{
        			var tickmarks = prop["chart.tickmarks"],tickSize = prop["chart.ticksize"],xMin = prop["chart.xmin"],x = ((x - xMin) / (xMax - xMin)) * (ca.width - this.gutterLeft - this.gutterRight),originalX = x,originalY = y;
        				 ...
        				 && typeof y[4] === 'number')
        			{
        				...
        				var y0 = this.getYCoord(y[0]),y1 = this.getYCoord(y[1]),y2 = this.getYCoord(y[2]),y3 = this.getYCoord(y[3]),y4 = this.getYCoord(y[4]),col1 = y[5],col2 = y[6],boxWidth = typeof y[7] == 'number' ? y[7] : prop["chart.boxplot.width"];
        			...
        			if (prop["chart.line.visible"] && typeof (y) == 'number' && !y0 && !y1 && !y2 && !y3 && !y4)
        			{
        				...
        				{
        					var graphWidth = ca.width - this.gutterLeft - this.gutterRight,graphheight = ca.height - this.gutterTop - this.gutterBottom,xVal = ((x - this.gutterLeft) / graphWidth) * xMax,yVal = ((graphheight - (yCoord - this.gutterTop)) / graphheight) * yMax;
        ...
        ;
        			...
        			this.DrawVBars = function ()
        		{
        			...
        			if (vbars)
        			{
        				...
        				for (var i = 0,len = vbars.length; i < len; i += 1)
        				{
        					...
        					{
        						value[0] = RG.parseDate(value[0]);
        					}
        					if (typeof value[1] == 'string')
        					{
        						value[1] = RG.parseDate(value[1]) - value[0];
        					}
        			...
        			this.getPoint = function (e)
        		{
        			...
        			var overHotspot = false;
        			var offset = prop["chart.tooltips.hotspot"];
        				...
        				for (var i = 0,len2 = this.coords[set].length; i < len2; ++i)
        				{
        					...
        					if (typeof (y) == 'number')
        					{
        						if (mouseX <= (x + offset) &&
        							...
        							mouseY >= (y - offset))
        						{
        							tooltip = RG.parseTooltipText(tooltip, 0);
        		...
        		this.getObjectByXY = function (e)
        		{
        			...
        			if (mouseXY[0] > (this.gutterLeft - 3)
        		 ...
        		 */
        		nexacro.RGraph.Scatter_Bubble = function (scatter, min, max, width, data)
        		{
        			...
        			this.draw =
        				this.Draw = function ()
        			{
        				var bubble_min = this.min,bubble_max = this.max,bubble_data = this.data,bubble_max_width = this.width;

        				// Keep a record of the bubble chart object
        				var obj_bubble = this,obj_scatter = this.scatter;
        				...
        				this.scatter.ondraw = function (obj)
        				{
        					...
        					for (var i = 0; i < obj.coords[0].length; ++i)
        					{
        						...
        						var r = ((bubble_data[i] - bubble_min) / (bubble_max - bubble_min)) * bubble_max_width,color = obj_scatter.data[0][i][2] ? obj_scatter.data[0][i][2] : obj_scatter.properties["chart.defaultcolor"];

        						co.beginPath();

        						grad = "gradient(" + (prop["chart.colors.bubble.graduated"] ? 'white' : (color + ":" + color)) + ")";
        						if (RG.ISRUNTIME)
        						{
        							grad = "gradient(" + (prop["chart.colors.bubble.graduated"] ? (color + ':white') : (color + ":" + color)) + ")";
        						}
        						colcol = RG.createRadialGradient(co,
        			...
        			if (data)
        			{
        				for (var dataset = 0; dataset < data.length; ++dataset)
        				{
        					for (var i = 0; i < this.data[dataset].length; ++i)
        					{
        						...
        						{
        							if (typeof (this.data[dataset][i][1][5]) == 'string')
        							{
        								this.data[dataset][i][1][5] = this.parseSingleColorForGradient(this.data[dataset][i][1][5]);
        							}
        							if (typeof (this.data[dataset][i][1][6]) == 'string')
        							{
        								this.data[dataset][i][1][6] = this.parseSingleColorForGradient(this.data[dataset][i][1][6]);
        							}
        						}
        		...
        		{
        			var obj = this,callback = arguments[2],opt = arguments[0] || {
