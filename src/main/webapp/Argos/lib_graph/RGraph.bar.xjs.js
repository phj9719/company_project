...
(function()
{
    return function(path)
    {
         ...
         */
        if (!nexacro.RGraph.Bar)
        {
        	nexacro.RGraph.Bar = function (conf)
        	{
        		// argument 0 : option
        		// 1 : rg common
        		// 2 : canvas id
        		...
        		this.properties =
        			...
        			'chart.background.image.background' : "", // hsji
        			'chart.background.image.gradation' : "",
        			...
        			'chart.labels.above.background' : [255, 255, 255, 0],
        			...
        			'chart.colors' : ['#86B5BC', '#E30513', '#1C1C1B', '#86BC24', '#E5007D', '#2F8DCD', '#F9D900', '#F6A200', '#BCBCBC'],
        			/*
        			 'chart.colors':                 [
        			...
        			'chart.shadow' : false,
        			'chart.shadow.color' : '#aaaaaa', // Gradients aren't supported for this color
        			...
        			'chart.highlight.fill' : [255, 255, 255, 0.4],
        			'chart.key' : null,
        			...
        			// 'chart.key.interactive.background.color' :[255,255,255,0.5],		// hsji
        		...
        		for (var i = 0; i < data.length; ++i)
        		{
        			if (typeof data[i] === 'object' && !RG.is_null(data[i]))
        			{
        		...
        		this.data = data;
        		this.original_data = RG.arrayClone(data);
        			...
        			this.Draw = function ()
        		{
        			...
        			if (RG.ISIE8)
        			{
        				...
        				this.set("chart.labels.above", true);
        			}
        			...
        			this.DrawAxes = function ()
        		{
        			...
        			if (prop["chart.noyaxis"] == false && !isSketch)
        			{
        				...
        				if (this.properties["chart.numyticks"] > 0)
        				{
        					...
        					{
        						co.moveTo(xpos + (yaxispos == 'left' ? 0 : 0), Math.round(ca.height - prop["chart.gutter.bottom"]));
        						co.lineTo(xpos + (yaxispos == 'left' ? -3 : 3), Math.round(ca.height - prop["chart.gutter.bottom"]));
        					}
        				}
        			...
        			this.Drawbars = function ()
        		{
        			...
        			var prevX = 0,prevY = 0,decimals = prop["chart.scale.decimals"];
        			...
        			for (var i = 0,len = this.data.length; i < len; i += 1)
        			{
        				...
        				if (shadow)
        				{
        					if (shadowColor)
        					{
        						...
        						RG.setElementCommonStyle(co, shadowColor, 3);
        					}
        					if (shadowBlur)
        					{
        						co.setElementShadowBlur(shadowBlur);
        					}
        					if (shadowOffsetX)
        					{
        						co.setElementShadowOffsetX(shadowOffsetX);
        					}
        					if (shadowOffsetY)
        					{
        						co.setElementShadowOffsetY(shadowOffsetY);
        					}
        				}
        				...
        				if (typeof this.data[i] == 'number')
        				{
        					...
        					if (barWidth < 0)
        					{
        						trace('[RGRAPH] Warning: you have a negative bar width. This may be caused by the chart.hmargin being too high or the width of the canvas not being sufficient.');
        					}
        					...
        					else if (variant == 'bar' || variant == '3d' || variant == 'glass' || variant == 'bevel')
        					{
        						...
        						if (variant == 'glass')
        						{
        							RG.filledCurvyRect(co, x + hmargin, y, barWidth, height, 3, this.data[i] > 0, this.data[i] > 0, this.data[i] < 0, this.data[i] < 0);
        							RG.strokedCurvyRect(co, x + hmargin, y, barWidth, height, 3, this.data[i] > 0, this.data[i] > 0, this.data[i] < 0, this.data[i] < 0);
        						}
        						else
        						{
        						...
        						if (variant == '3d')
        						{
        							...
        							co.lineTo(x + hmargin + barWidth + prop["chart.variant.threed.offsetx"],
        								...
        								Math.min(y - prop["chart.variant.threed.offsety"] + height, ca.height - this.gutterBottom)
        )
        							);
        							...
        							co.lineTo(
        								...
        								this.data[i] < 0 && (y - prop["chart.variant.threed.offsety"] + height) < this.getYCoord(0)
        								 ? this.getYCoord(0)
        								 : this.data[i] > 0 ? y - prop["chart.variant.threed.offsety"] + height : Math.min(y - prop["chart.variant.threed.offsety"] + height, ca.height - this.gutterBottom)
        							);
        							// BL
        							co.lineTo(x + hmargin + barWidth, y + height);
        							co.lineTo(x + hmargin + barWidth, y);
        						...
        						{
        							var grad = RG.createLinearGradient(co, x + hmargin, y, x + hmargin + (barWidth / 2), y, "gradient(#ffffffdd:#ffffff88)");
        				...
        				else if (this.data[i] && typeof (this.data[i]) == 'object' && prop["chart.grouping"] == 'stacked')
        				{
        					...
        					for (var j = 0; j < dataset.length; ++j)
        					{
        						...
        						{
        							co.fillStyle = colors[j] ? colors[j] : colors[0];
        							RG.setElementCommonStyle(co, co.fillStyle, 1);
        						}

        						if (!colors[sequentialColorIndex])
        						{
        							sequentialColorIndex = 0;
        						}
        						if (prop["chart.colors.sequential"])
        						{
        							var colcol = colors[sequentialColorIndex++];
        							if (!colcol)
        							{
        								colcol = colors[0];
        							}
        						...
        						var totalHeight = (RG.array_sum(dataset) / this.scale2.max) * (ca.height - hmargin - this.gutterTop - this.gutterBottom);
        					...
        					if (shadow)
        					{
        						...
        						for (var k = 0; k < redrawCoords.length; ++k)
        						{
        							if (shadowColor)
        							{
        								co.shadowColor = shadowColor;
        							...
        							{
        								co.shadowColor = co, redrawCoords[k][4];
        								RG.setElementCommonStyle(co, shadowColor, 3);
        							}
        							if (shadowBlur)
        							{
        							...
        							/*
        							 co.strokeStyle = strokeStyle;
        							 RG.setElementCommonStyle(co,strokeStyle,2);
        							 RG.setElementCommonStyle(co,redrawCoords[k][4],1);
        					...
        					for (var j = 0; j < this.data[i].length; ++j)
        					{
        						...
        						if (variant == '3d')
        						{
        							...
        							co.lineTo(
        								...
        								this.data[i][j] < 0 ? (this.getYCoord(0) + Math.abs(height) - prop["chart.variant.threed.offsety"]) : this.getYCoord(0) - height - prop["chart.variant.threed.offsety"]
        							);
        			...
        			this.DrawLabels = function ()
        		{
        			...
        			if (RG.ISIE8)
        			{
        				this.set('chart.text.angle', 0);
        			}

        			var text_angle = prop["chart.text.angle"],text_size = prop["chart.text.size"],labels = prop["chart.labels"];
        			...
        			{
        				var yOffset = Number(prop["chart.labels.offsety"]),xOffset = Number(prop["chart.labels.offsetx"]),bold = prop["chart.labels.bold"];
        			...
        			this.Drawlabels_top = function ()
        		{
        			...
        			if (prop["chart.xaxispos"] == 'top')
        			{
        				...
        				{
        					var labels = RG.array_reverse(prop["chart.ylabels.specific"]);
        				...
        				if (prop["chart.ymin"] != 0 || prop["chart.noxaxis"] || prop["chart.scale.zerostart"])
        				{
        					RG.Text2(this, {
        						...
        						'text' : (this.scale2.min != 0 ? '-' : '') + RG.numberFormat(this, (this.scale2.min.toFixed((this.scale2.min === 0 ? 0 : prop["chart.scale.decimals"]))), units_pre, units_post),
        						'valign' : 'center',
        						'halign' : align,
        						'bordered' : boxed,
        						'tag' : 'scale'
        			...
        			this.getBar = function (e)
        		{
        			...
        			var obj = this;
        			var mouseXY = RG.getMouseXY(e),mouseX = mouseXY[0],mouseY = mouseXY[1],canvas = obj.canvas,context = obj.context,coords = obj.coords;
        			...
        			for (var i = 0,len = coords.length; i < len; i += 1)
        			{
        				...
        				if (mouseX >= left && mouseX <= (left + width) && mouseY >= top && mouseY <= (top + height))
        				{
        					...
        					// trace(mouseX + ":" + left + ":" + (left + width));
        					// trace(mouseY + ":" + top + ":" + (top + height));
        		...
        		this.getYCoord = function (value)
        		{
        			...
        			var co = this.context,ca = this.canvas,prop = this.properties;
        		...
        		{
        			var labels = prop["chart.labels.above"],specific = prop["chart.labels.above.specific"],color = prop["chart.labels.above.color"],background = prop["chart.labels.above.background"],decimals = prop["chart.labels.above.decimals"],size = prop["chart.labels.above.size"],angle = -1 * prop["chart.labels.above.angle"],unitsPre = prop["chart.labels.above.units.pre"],unitsPost = prop["chart.labels.above.units.post"],coords = this.coords,coords2 = this.coords2,data = this.data,ldata = RG.arrayLinearize(this.data),offset = prop["chart.labels.above.offset"],text_font = prop["chart.text.font"],text_size = prop["chart.text.size"],border = prop["chart.labels.above.border"],grouping = prop["chart.grouping"];
        				...
        				for (var i = 0,len = data.length,sequentialIndex = 0; i < len; i += 1)
        				{
        					...
        					if (typeof data[i] === 'number' && data[i] >= 0)
        					{
        						...
        						RG.text2(this, {
        							...
        							'bounding.stroke' : [255, 255, 255, 0], // hsji	[0,0,0,0]
        							'aboveborder' : border,
        							'tag' : 'labels.above'
        						});
        		...
        		this.colorWave = function ()
        		{
        			...
        			var framesperbar = opt.frames / 2,frame = -1,callback = arguments[1] || function ()
        			{
        		...
        		this.grow = function ()
        		{
        			...
        			if (RG.isArray(opt.data))
        			{
        				...
        				for (var i = 0; i < opt.data.length; ++i)
        				{
        					if (typeof opt.data[i] === 'object')
        					{
        						for (var j = 0; j < opt.data[i].length; ++j)
        						{
        							if (typeof opt.data[i][j] === 'string' && opt.data[i][j].match(/(\+|\-)([0-9]+)/))
        							{
        								if (RegExp.$1 === '+')
        								{
        									var val = this.original_data[i][j] + parseInt(RegExp.$2);
        									if (!val)
        									{
        										val = 0;
        									}
        					...
        					else if (typeof opt.data[i] === 'string' && opt.data[i].match(/(\+|\-)([0-9]+)/))
        					{
        						if (RegExp.$1 === '+')
        						{
        							var val = this.original_data[i] + parseInt(RegExp.$2);
        							if (!val)
        							{
        								val = 0;
        							}
        			...
        			if (prop["chart.ymax"] == null)
        			{
        				...
        				for (var i = 0; i < obj.data.length; ++i)
        				{
        					...
        					{
        						ymax = Math.max(ymax, Math.abs(RG.arraySum(this.data[i])));
        					}
        					else if (RG.isArray(this.data[i]) && prop["chart.grouping"] === 'grouped')
        					{
        						...
        						ymax = Math.max(ymax, Math.abs(RG.arrayMax(group)));
        					}
        					else
        					{
        			...
        			var iterator = function ()
        			{
        				try
        				{
        					...
        					for (var j = 0,len = obj.original_data.length; j < len; ++j)
        					{
        						if (typeof obj.data[j] === 'object' && !RG.isNull(obj.data[j]))
        						{
        							for (var k = 0,len2 = obj.data[j].length; k < len2; ++k)
        							{
        								...
        								{
        									var val = easingMultiplier * obj.original_data[j][k];
        									if (!val)
        									{
        										val = 0;
        									}
        								...
        								else if (opt.data && opt.data.length === obj.original_data.length)
        								{
        									...
        									var val = (easingMultiplier * diff) + obj.original_data[j][k];
        									if (!val)
        									{
        										val = 0;
        									}
        						...
        						else
        						{
        							if (obj.firstDraw || !opt.data)
        							{
        								var val = easingMultiplier * obj.original_data[j];
        								if (!val)
        								{
        									val = 0;
        								}
        							...
        							else if (opt.data && opt.data.length === obj.original_data.length)
        							{
        								...
        								var val = (easingMultiplier * diff) + obj.original_data[j];
        								if (!val)
        								{
        									val = 0;
        								}
        					...
        					RG.clear(obj.canvas);
        					RG.redrawCanvas(obj.canvas);
        					RG.effectCanvasShow(obj.canvas, false, 30);
        		...
        		{
        			var coords = this.coords,color = prop["chart.errorbars.color"] || 'black',default_halfwidth = Math.min(prop["chart.errorbars.capped.width"], coords[0][2]) / 2,x = 0,errorbars = prop["chart.errorbars"],length = 0;
        	 ...
        	 * This is the combined bar and Line class which makes creating bar/line combo charts a little bit easier *
        	 /*********************************************************************************************************/
        	nexacro.RGraph.CombinedChart = function (RG)
        	{
        		/**
        	 ...
        	 */
        	nexacro.RGraph.CombinedChart.prototype.add =
        		nexacro.RGraph.CombinedChart.prototype.Add = function (obj)
        	{
        	 ...
        	 */
        	nexacro.RGraph.CombinedChart.prototype.draw =
        		nexacro.RGraph.CombinedChart.prototype.Draw = function ()
        	{
