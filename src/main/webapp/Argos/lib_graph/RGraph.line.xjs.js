...
(function()
{
    return function(path)
    {
         ...
         */
        if (!nexacro.RGraph.Line)
        {
        	nexacro.RGraph.Line = function (conf)
        	{
        		// argument 0 : option
        		// 1 : rg common
        		// 2 : canvas id
        		...
        		this.properties =
        			...
        			'chart.background.barcolor1' : [255, 255, 255, 0.5],
        			'chart.background.barcolor2' : [255, 255, 255, 0.5],
        			...
        			'chart.colors' : ['red', '#0f00f0', '#00f00f', '#f0ff0f', '#ff0ff0', '#0ff0ff', 'green', 'pink', 'blue', 'black'],
        			'chart.hmargin' : 0,
        			'chart.tickmarks.dot.stroke' : 'white',
        			'chart.tickmarks.dot.fill' : null,
        			'chart.tickmarks.dot.linewidth' : 3,
        			'chart.tickmarks.stroke.color' : null,
        			...
        			'chart.shadow.blur' : 3,
        			'chart.shadow.color' : [128, 128, 128, 0.5],
        			'chart.tooltips' : null,
        			...
        			'chart.key.interactive.highlight.chart.stroke' : [255, 0, 0, 0.3],
        			...
        			'chart.xlabels.inside.color' : [255, 255, 255, 0.5],
        			'chart.noaxes' : false,
        			'chart.noyaxis' : false,
        			'chart.noxaxis' : false,
        		...
        		{
        			if (typeof conf.data[0] === 'number' || RG.isNull(conf.data[0]))
        			{
        				this.original_data[0] = RG.arrayClone(conf.data);

        				// } else if (typeof conf.data[0] === 'object' && !RG.isNull(conf.data[0])) {
        			}
        			else
        			{
        				for (var i = 0; i < conf.data.length; ++i)
        				{
        					this.original_data[i] = RG.arrayClone(conf.data[i]);
        				}
        			}
        		...
        		else
        		{
        			for (var i = 1; i < arguments.length; ++i)
        			{
        					 ...
        					 && arguments[1][0].length)
        				{
        					...
        					{
        						tmp[i] = RG.array_clone(arguments[1][i]);
        					}

        					for (var j = 0; j < tmp.length; ++j)
        					{
        						this.original_data[j] = RG.array_clone(tmp[j]);
        					}
        				}
        				else
        				{
        					this.original_data[i - 1] = RG.array_clone(arguments[i]);
        				}
        			}
        		}
        		 ...
        		 */
        		this.data_arr = RG.arrayLinearize(this.original_data);
        			...
        			this.Set = function (name)
        		{
        			 ...
        			 */
        			if (name == 'chart.linewidth' && RG.ISCHROME)
        			{
        				if (value == 1)
        				{
        					value = 1.01;
        				}
        				else if (RG.is_array(value))
        				{
        			...
        			this.Draw = function ()
        		{
        			...
        			if (prop["chart.ymax"])
        			{
        				...
        				if (!prop["chart.outofbounds"])
        				{
        					...
        					{
        						if (RG.isArray(this.data[dataset]))
        						{
        			...
        			for (var i = 0,j = 0,len = this.data.length; i < len; i++, j++)
        			{
        				...
        				if (this.data[i])
        				{
        					this.drawLine(
        						this.data[i],
        						this.GetColor(j), // prop['chart.colors'][j],
        						fill,
        						this.getLineWidth(j),
        						tickmarks,
        						i
        			...
        			if (prop["chart.filled"] && prop["chart.filled.accumulative"] && !prop["chart.curvy"])
        			{
        				for (var i = 0; i < this.coords2.length; ++i)
        				{
        					...
        					co.strokeStyle = !this.hidden(i) ? prop["chart.colors"][i] : [0, 0, 0, 0.001];
        				...
        				if (prop["chart.tickmarks"])
        				{
        					...
        					{
        						co.beginPath();
        						var cc = prop["chart.colors"][i] ? prop["chart.colors"][i] : prop["chart.colors"][0] ? prop["chart.colors"][0] : co.strokeStyle;
        						co.strokeStyle = cc;
        						RG.setElementCommonStyle(co, co.strokeStyle, 2);
        						if (this.coords2[i])
        						{
        				...
        				for (var i = 0,len = this.coords2.length; i < len; i += 1)
        				{
        					if (this.coords2[i])
        					{
        						for (var j = 0,len2 = this.coords2[i].length; j < len2; ++j)
        						{
        							if (typeof (this.coords2[i][j]) == 'object' && typeof (this.coords2[i][j][0]) == 'number' && typeof (this.coords2[i][j][1]) == 'number')
        							{
        								var tickmarks = typeof prop["chart.tickmarks"] == 'object' && !RG.is_null(prop["chart.tickmarks"]) ? prop["chart.tickmarks"][i] : prop["chart.tickmarks"];
        								co.strokeStyle = prop["chart.colors"][i];
        								RG.setElementCommonStyle(co, co.strokeStyle, 2);
        								this.DrawTick(
        									this.coords2[i],
        			...
        			if (prop["chart.filled"] && prop["chart.filled.range"] && this.data.length == 2)
        			{
        				...
        				co.setElementLineWidth(co.lineWidth);
        				co.strokeStyle = this.hidden(0) ? [0, 0, 0, 0.001] : prop["chart.colors"][0];
        				RG.setElementCommonStyle(co, co.strokeStyle, 2);
        				...
        				{
        					co.strokeStyle = this.hidden(1) ? [0, 0, 0, 0.001] : prop["chart.colors"][1];
        					RG.setElementCommonStyle(co, co.strokeStyle, 2);
        				}
        			...
        			this.DrawAxes = function ()
        		{
        			...
        			if (prop["chart.noxaxis"] == false && prop["chart.numxticks"] > 0)
        			{
        				...
        				for (x = this.gutterLeft + (prop["chart.yaxispos"] == 'left' ? xTickInterval : 0); x <= (ca.width - this.gutterRight + 1); x += xTickInterval)
        				{
        					...
        					{
        						var yStart = this.getYCoord(prop["chart.ymin"]) - (prop["chart.ymin"] >= 0 ? 0 : 3),yEnd = this.getYCoord(prop["chart.ymin"]) + 3;
        					...
        					{
        						var yStart = Math.round((this.gutterTop + (this.grapharea / 2))) - 3,yEnd = yStart + 6;
        					}
        					else if (prop["chart.xaxispos"] == 'bottom')
        					{
        						var yStart = this.getYCoord(0) - (prop["chart.ymin"] !== 0 ? 3 : 0),yEnd = this.getYCoord(0) - (x % 60 == 0 ? prop["chart.largexticks"] * prop["chart.tickdirection"] : prop["chart.smallxticks"] * prop["chart.tickdirection"]);
        						yEnd += 0;
        					}
        					else if (prop["chart.xaxispos"] == 'top')
        					{
        			...
        			this.DrawLabels = function ()
        		{
        			...
        			co.strokeStyle = strokecol = 'black';
        			RG.setElementCommonStyle(co, co.strokeStyle, 2);
        			co.fillStyle = fillcol = prop["chart.text.color"];
        			RG.setElementCommonStyle(co, co.fillStyle, 1);
        			co.lineWidth = 1;
        			co.setElementLineWidth(co.lineWidth);
        			...
        			if (prop["chart.ylabels"] && prop["chart.ylabels.specific"] == null)
        			{
        				...
        				else
        				{
        					...
        					{
        						var xx = this.scale2.labels[this.scale2.labels.length - (i + 1)];
        						RG.setElementCommonStyle(co, co.fillStyle, 1);
        						for (var i = 0,len = this.scale2.labels.length; i < len; ++i)
        						{
        							RG.text2(this, {
        					 ...
        					 && prop["chart.noendytick"] == false)
        				{
        					RG.text2(this, {
        						...
        						'size' : text_size,
        						'x' : xpos + offsetx,
        						'y' : prop["chart.xaxispos"] == 'top' ? this.gutterTop + offsety : (ca.height - this.gutterBottom),
        						'text' : prop["chart.units.pre"] + Number(0).toFixed(prop["chart.scale.decimals"]) + prop["chart.units.post"] + offsety,
        						'valign' : 'center',
        						'halign' : align,
        						'bounding' : bounding,
        						'boundingFill' : bgcolor,
        			...
        			{
        				var yOffset = 5,bordered = false,bgcolor = null;
        				 ...
        				 * Text angle
        				 */
        				var angle = 0,valign = 'top',halign = 'center',bold = prop["chart.labels.bold"];
        				...
        				var numLabels = prop["chart.labels"].length,offsetx = prop["chart.labels.offsetx"],offsety = prop["chart.labels.offsety"];
        			...
        			this.DrawLine = function (lineData, color, fill, linewidth, tickmarks, index)
        		{
        			...
        			if (lineData)
        			{
        				for (var i = 0,len = lineData.length; i < len; i += 1)
        				{
        					...
        					// Null data points, and a special case for this bug:http://dev.RG.net/tests/ymin.html
        					if (lineData[i] == null
        			...
        			if (prop["chart.filled"] && !prop["chart.filled.range"] && !prop["chart.curvy"])
        			{
        				...
        				co.fillStyle = !this.hidden(index) ? fill : [0, 0, 0, 0.001];
        				RG.setElementCommonStyle(co, co.fillStyle, 1);
        			 ...
        			 */
        			// if (false && RG.ISCHROME && prop['chart.shadow'] && prop['chart.chromefix'] && prop['chart.shadow.blur'] > 0) {
        			//
        			...
        			this.DrawTick = function (lineData, xPos, yPos, color, isShadow, prevX, prevY, tickmarks, index, dataset)
        		{
        			...
        			co.setElementLineWidth(co.lineWidth);
        			co.strokeStyle = isShadow ? prop["chart.shadow.color"] : prop["chart.tickmarks.stroke.color"] ? prop["chart.tickmarks.stroke.color"] : color;
        			RG.setElementCommonStyle(co, co.strokeStyle, 2);
        			// co.fillStyle   = isShadow ? prop['chart.shadow.color'] : co.strokeStyle;
        			co.fillStyle = isShadow ? prop["chart.shadow.color"] : color; //hsji
        			RG.setElementCommonStyle(co, co.fillStyle, 1);
        			...
        			else if (tickmarks == 'triangle' || tickmarks == 'filledtriangle' || (tickmarks == 'endtriangle' && (index == 0 || index == (lineData.length - 1))))
        			{
        				...
        				{
        					// co.fillStyle = isShadow ? prop['chart.shadow.color'] : co.strokeStyle;
        					co.fillStyle = isShadow ? prop["chart.shadow.color"] : color;
        					RG.setElementCommonStyle(co, co.fillStyle, 1);
        				}
        				else
        				{
        			...
        			else if (typeof tickmarks === 'string' &&
        				(
        ...
        ))
        			{
        				...
        				img.onload = function ()
        				{
        					if (prop["chart.tickmarks.image.halign"] === 'center')
        					{
        						xPos -= (this.width / 2);
        					}
        					if (prop["chart.tickmarks.image.halign"] === 'right')
        					{
        						xPos -= this.width;
        					}

        					if (prop["chart.tickmarks.image.valign"] === 'center')
        					{
        						yPos -= (this.height / 2);
        					}
        					if (prop["chart.tickmarks.image.valign"] === 'bottom')
        					{
        						yPos -= this.height;
        					}
        			...
        			this.RedrawLine = function (coords, color, linewidth, index)
        		{
        			...
        			{
        				this.DrawCurvyLine(coords, this.hidden(index) ? [0, 0, 0, 0.001] : color, linewidth, index);
        				return;
        			}
        			...
        			this.DrawAboveLabels = function ()
        		{
        ...
        ) ? prop["chart.labels.above.border"] : true),offsety = prop["chart.labels.above.offsety"] + size,specific = prop["chart.labels.above.specific"];
        			...
        			for (var i = 0,len = this.coords.length; i < len; i += 1)
        			{
        				...
        				RG.text2(this, {
        					...
        					'bounding' : true,
        					'boundingFill' : bgcolor,
        					'boundingStroke' : border ? 'black' : [0, 0, 0, 0.001],
        					'tag' : 'labels.above'
        				});
        			}
        		};
        			...
        			this.Highlight = function (shape)
        		{
        			if (prop["chart.tooltips.highlight"])
        			{
        				...
        				{
        					var obj = shape.object,color = prop["chart.colors"][shape.dataset];

        					// Clear a space in white first for the tickmark
        					RG.path2(obj.context, 'b a % % 13 0 6.2830 false f #ffffffdd',
        						shape.x,
        						shape.y
        					);

        					var cc = color;
        					if (typeof (color) == "string" && color.indexOf("#") >= 0 && color.length < 8)
        					{
        						cc = color + "88";
        					}
        		...
        		this.getObjectByXY = function (e)
        		{
        			...
        			if ((mouseXY[0] > prop["chart.gutter.left"] - 5)
        			...
        			this.SetShadow = function (i)
        		{
        			...
        			if (prop["chart.shadow"])
        			{
        				...
        				if (typeof (shadowColor) == 'object')
        				{
        					if (shadowColor[0] && typeof (shadowColor[0]) == "number")
        					{
        						if ((shadowColor[0] >= 0 && shadowColor[0] <= 256) &&
        							(shadowColor[1] >= 0 && shadowColor[1] <= 256))
        						{
        							shadowColor = RG.rgba2hex(shadowColor);
        						}
        					}
        				}
        				/**
        				 ...
        				 */
        				if (typeof (shadowColor) == 'object' && shadowColor[i])
        				{
        		...
        		this.drawErrorbars = function ()
        		{
        			...
        			var coords = this.coords,x = 0,errorbars = prop["chart.errorbars"],length = 0;
        			...
        			for (var i = 0; i < coords.length; ++i)
        			{
        				var halfwidth = prop["chart.errorbars.capped.width"] / 2 || 5,color = prop["chart.errorbars.color"] || 'black';
        				...
        				if (typeof errorbars === 'number' || typeof errorbars[i] === 'number')
        				{
        					...
        					{
        						var positiveLength = this.getYCoord(this.min) - this.getYCoord(this.min + errorbars),negativeLength = positiveLength;
        					}
        					else
        					{
        						var positiveLength = this.getYCoord(this.min) - this.getYCoord(this.min + errorbars[i]),negativeLength = positiveLength;
        					}
        				...
        				{
        					var positiveLength = this.getYCoord(this.min) - this.getYCoord(this.min + errorbars[i][0]),negativeLength = this.getYCoord(this.min) - this.getYCoord(this.min + errorbars[i][1]);
        			...
        			this.trace2 = function ()
        		{
        			...
        			obj.Set('animation.trace.clip', 0.01);
        			...
        			this.unfoldFromCenterTrace2 = function ()
        		{
        			...
        			},frames = opt.frames || 10,frame = 0,data = RG.arrayClone(obj.original_data),callback = arguments[1] || function ()
        			{
        			};
        		...
        		this.unfoldFromCenter = function ()
        		{
        			...
        			// Draw the chart once to get the scale values
        			obj.canvas.set_visible(false);
        			obj.Draw();
        			var max = obj.scale2.max;
        			RG.clear(obj.canvas);
        			obj.canvas.set_visible(true);
        			...
        			function unfoldFromCenter ()
        			{
        				...
        				if (--frames > 0)
        				{
        					RG.Effects_updateCanvas(unfoldFromCenter);
        				}
        				else
        				{
