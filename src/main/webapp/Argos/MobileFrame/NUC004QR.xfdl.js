(function()
{
    return function()
    {
        ...
        // User Script
        this.addIncludeScript("NUC004QR.xfdl","lib::sct_com.xjs");
        this.addIncludeScript("NUC004QR.xfdl","lib::pgm_com.xjs");
        this.addIncludeScript("NUC004QR.xfdl","lib::mpgm_com.xjs");
        this.registerScript("NUC004QR.xfdl", function() {
        /***********************************************************************
         * 화면ID    	: NUC004QR
         * 화면명   	: 공지사항
         * 화면설명 	: 모바일에서 요양원 공지사항을 확인하는 화면
         * 작성일    	: 2021-07-06
         * 작성자   	: 조기종
         * 수정이력 	:
         ***********************************************************************
         *     	수정일     	수정자   	내용
         *		2021-09-23		조기종		파일 다운로드 추가 & 아코디언 수정
         ***********************************************************************
         ***********************************************************************
         */
        ...
        this.form_onload = function (obj, e)
        {
        	...
        	// 네이티브에서 다운로드를 위해 네이티브 통신 오브젝트 생성
        ...
        this.fn_search = function ()
        {
        	...
        	const argument = "method=getList "
        		+ "sqlId=NUC004QR.NUC004QR_DS_LIST_S ";
        ...
        {
        	const i = obj.id.split("_")[2];
        	const objDiv = this.div_acco.form.components["div_detail_" + i];
        	const objmDiv = this.div_acco.form.components["div_title_" + i];
        	const objBtn = objmDiv.form.components["btn_isOpen_" + i];
        	...
        	if(nHeight == 0)
        	{
        		...
        		// 스크롤바 max값이 곧 InnerForm의 Height와 같음 (여유로 5정도 더 늘림)
        		nTileHeight = objDiv.form.vscrollbar.max + 5;
        ...
        this.fnMakeAccordionForm = function(objDiv, objDs)
        {
        	...
        	for(let i = 0; i < nCount; i++)
        	{
        		...
        		objComp.set_font("normal 700 14pt/normal 'Noto Sans KR'");
        		...
        		objComp.set_font("normal 700 18pt/normal 'Noto Sans KR'");
        ...
        this.fnMakeAccordionSubForm = function(i, objDiv, objDs)
        {
        	...
        	const objSize = nexacro.getTextSize(title + "\n\n\n" + content, "normal 500 15pt/normal 'Noto Sans KR'", 420, "char");
        	const objaa = Sct.Util.appendComma(objSize.ny, 0);
        	nCompHeight = parseInt(objaa) + 30;

        	objComp = new Static("sta_content_" + i, nLeft, nTop, 420, nCompHeight, "5%", null);
        	objComp.set_text(title + "\n\n\n" + content);
        	objComp.set_color("#7d7f97");
        	objComp.set_wordWrap("char");
        	objComp.set_font("normal 500 15pt/normal 'Noto Sans KR'");
        	...
        	if(fileList != null && fileList != '')
        	{
        		...
        		for(i = 0; i < fileArr.length; i++)
        		{
        			if(i == 0)
        			{
        				...
        				// 파일명의 글자수가 25자 이상일 경우 ...으로 말줄임
        				const length = 25;
        				let fileName;
        				if(fileArr[i].length > length)
        				{
        					fileName = fileArr[i].substr(0, length - 2) + "...";
        				}
        				else
        				{
        					fileName = fileArr[i];
        				...
        				objComp.set_text(fileName);
        				objComp._fileName = fileArr[i];
        ...
        this.fileDownClick = function(obj, e)
        {
        	...
        	const fileName = obj._fileName;
        	this.downloadFile(fileName, fileRealName);
        };

        // 파일 다운로드
        // 안드로이드에서는 jsp 경유가 안되므로 네이티브에서 다운로드
        this.downloadFile = function(fileName, fileRealName)
        {
         	...
         	const fileUrl = svcUrl + path + fileRealName;
        	...
        	this.standardPlugin.callMethod("FILEDOWN", param);
        };
        ...
        this.loadIncludeScript("NUC004QR.xfdl");
