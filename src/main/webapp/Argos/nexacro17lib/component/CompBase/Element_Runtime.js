...
if (nexacro._Browser == "Runtime") {
	...
	if (!nexacro.Element) {
		...
		_pImageElement.setElementImage = function (image, width, height, stretch, imageviewmode, comp) {
			if ((this.image != image) || (this.width != width || this.height != height || this.stretch != stretch)) {
				this.image = image;
				var handle = this.handle;

				if (handle && image) {
					if (comp && (comp.parent instanceof nexacro.ImageViewer) && stretch == "fixaspectratio") {
						nexacro.__setElementHandleImageUrlObject(handle, image, comp.width, comp.height, stretch, imageviewmode);
					}
					else {
						nexacro.__setElementHandleImageUrlObject(handle, image, width, height, stretch, imageviewmode);
					}
				}
				else {
