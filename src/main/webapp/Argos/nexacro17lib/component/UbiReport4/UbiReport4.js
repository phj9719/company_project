if (!nexacro.UbiViewer) {
	...
	_pUbiViewer.invisibleexporttypes = "";
...
//	_pUbiViewer._ubi_callback_id = "";
	...
	_pUbiViewer.set_invisibleexporttypes = function(v) {
		this.invisibleexporttypes = v;
	};
	...
	_pUbiViewer.execRetrieve = function(parent, callback, strUrl, isCallUrl) {
		...
		if (_ubi_Browser == "Runtime")
		{
			...
			if (plugin_elem)
			{
				...
				// ë¤ë¥¸ ííì ë°ì´íìì ë°ë ê²½ì° ì´ê¸°í ìì
				plugin_elem._plugin_object.callMethod("SetVariable", "initData", "true");
				// ocx ëª¨ë ìì ì¼ë¡ ì¸í´ íë¨ ì´ê¸°í ë´ì© ì­ì 
				...
				// íì¼ ì ì¥ ëª©ë¡ ì ë¦¬
				var invisibletypes = "Hwp,Text,Urf,Data";
				if(this.invisibleexporttypes != "") {
					invisibletypes += ("," + this.invisibleexporttypes);
				}
				plugin_elem._plugin_object.setProperty("invisibleexporttypes", invisibletypes);
				
				// Dataset ì¤ì 
				if (dsinfos != "")
				{
					// ìë²ì ë°ì´í°ìì ì´ì©í  ê²½ì°
...
//					plugin_elem._plugin_object.setProperty("isLocalFile", nexacro._toBoolean('true'));
		...
		{
			// runtimeì¼ ê²½ì° runtimedata ìì±
			var runtimedata = "";
			var streamdata = "";
			if (this.designDataset)
			{
				// dataset SSV ì ì¡ì íìí ì ë³´ ìì±
				var ssvlist = [];
			...
			// ì¡°íì  Nexacro UbiViewerì ì¤ì ë ë³ìë¥¼ viewerì ì¤ì 
	...
	// ëìì¸ ì ë³´ë¥¼ ë´ê³ ìë dataset ìì±
...
*/
	...
	_pUbiViewer.addTransaction = function (parent, strSvcID, strURL, strInDatasets, strOutDatasets, strArgument, nDataType)
    {
		...
		// Dataê° encoding ëì´ìì ê²½ì° Decoding
    ...
    _pUbiViewer.on_create_contents = function() 
	{
		if (_ubi_Browser == "Runtime")
		{
			//alert('íì¬ ì§ìíì§ ìë ê¸°ë¥ìëë¤.');
			//return;
	...
	_pUbiViewer.addEventHandler = function (evt_id, func, target) {
		if (_ubi_Browser == "Runtime")
		{
			...
			if (this.viewer && this.viewer.events) {
				...
				} else if (evt_id == 'PrintClicked') {
					this.viewer.events.printClicked = func;
				} else if (evt_id == 'ExportClicked') {
					this.viewer.events.exportClicked = func;
				}
			}
		}
	};
