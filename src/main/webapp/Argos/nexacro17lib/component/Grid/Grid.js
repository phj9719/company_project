...
if (!nexacro.Grid) {
	...
	_pGrid.on_dsnotify_onload = function (obj, e) {
		...
		if (_reason == 0 || _reason == 1 || _reason == 2 || _reason == 3) {
            ...
            {
                if (this.autosizingtype != "none")  // 여기로 탈 경우 성능저하가 일어남.
                {
	...
	_pGrid._getTempCell = function (cellidx, rowidx, parent_cellinfo) {
		var bandid = this._getBandId(rowidx);
		var gridrow = this._getGridRow(rowidx);
	...
	_pGrid._getCellStyleInfo = function (cellidx, prop, datarow, selected, parent_cellinfo, autosizing) {
		...
		var cells = this._getTempCell(cellidx, datarow, parent_cellinfo);
		var cell = cells.cell;
		var subcell = cells.subcell;
		var target_cell = subcell ? subcell : cell;

		if (autosizing) {
			cell._updateAllEx(prop);
		}
		else {
			cell._updateAll();
		}
