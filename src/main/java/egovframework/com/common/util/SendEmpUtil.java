...
public class SendEmpUtil {
	...
	public void sendMainBatch() {
		...
		log.info("empBatch() Start!");
		TransactionDefinition transactionDefinition = new DefaultTransactionDefinition();
		TransactionStatus transactionStatus = transactionManager.getTransaction(transactionDefinition);

		try {
			
			//List<Map<String, Object>> nsHomeList = commonDao.getList("CM_BATCH_EMP.SCT_EMP_S");
			
			//if(nsHomeList.size()>0) {
				//for(int i = 0; i<nsHomeList.size(); i++){
				Map<String, Object> nsHomeMap = new HashMap<>();
				commonDao.updateData("CM_BATCH_EMP.SCT_EMP_U", nsHomeMap);
				commonDao.updateData("CM_BATCH_EMP.NS_EMP_U", nsHomeMap);
				//}	
			//}
