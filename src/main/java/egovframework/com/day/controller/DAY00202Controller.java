...
@Controller
public class DAY00202Controller {
	
	private Logger log = LoggerFactory.getLogger(DAY00202Controller.class);
	
	@Resource(name="DAY00202Service")
	private DAY00202Service DAY00202Service;
	
	@RequestMapping(value = "/DAY00202Controller.do")
	public NexacroResult operate(HttpServletRequest request) {
    	...
    	try {
			...
			switch (method) {
				case "save" ://저장(삽입,수정)
					result = DAY00202Service.save(inVar, inDataSet);
					break;
				case "recordCnt" ://저장(삽입,수정)
					DAY00202Service.recordCnt(inVar, inDataSet, outMap);
					break;
			}
