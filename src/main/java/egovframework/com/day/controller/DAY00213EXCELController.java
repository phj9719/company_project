...
@Controller
public class DAY00213EXCELController {
	
	private Logger log = LoggerFactory.getLogger(DAY00213EXCELController.class);
	
	@Resource(name="DAY00213EXCELService")
	private DAY00213EXCELService DAY00213EXCELService;
	
	@RequestMapping(value = "/DAY00213EXCEL.do")
	public NexacroResult save(HttpServletRequest request, HttpServletResponse response) {
    	...
    	try {
			...
			switch (method) {
				case "excelDown" :
					result = DAY00213EXCELService.excelDown(inVar, inDataSet);
					break;
			}
