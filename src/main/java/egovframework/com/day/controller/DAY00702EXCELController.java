...
@Controller
public class DAY00702EXCELController {
	
	private Logger log = LoggerFactory.getLogger(DAY00702EXCELController.class);
	
	@Resource(name="DAY00702EXCELService")
	private DAY00702EXCELService DAY00702EXCELService;
	
	@RequestMapping(value = "/DAY00702EXCEL.do")
	public NexacroResult save(HttpServletRequest request, HttpServletResponse response) {
    	...
    	try {
			...
			switch (method) {
				case "formDown" :
					result = DAY00702EXCELService.formDown(inVar, inDataSet, outDataSet);
					break;
				case "excelDown" :
					result = DAY00702EXCELService.excelDown(inVar, inDataSet);
					break;
			}
