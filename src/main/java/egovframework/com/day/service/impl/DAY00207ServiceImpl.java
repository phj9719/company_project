...
@Service("DAY00207Service")
public class DAY00207ServiceImpl extends EgovAbstractServiceImpl implements DAY00207Service {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED) {		
					log.info("인서트 데이터");
				 	commonDao.insertData("DAY00207.DAY00207_DS_MEDI_I", map);
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
					log.info("업데이트 데이터");
				 	commonDao.updateData("DAY00207.DAY00207_DS_MEDI_U", map);
				}
			}
