...
import egovframework.com.day.service.DAY00713Service;
import egovframework.com.rte.nexacro.NexacroConstant;
import egovframework.rte.fdl.cmmn.EgovAbstractServiceImpl;

@Service("DAY00713Service")
public class DAY00713ServiceImpl extends EgovAbstractServiceImpl implements DAY00713Service {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED) {
					List<Map<String, Object>> duplRecord = commonDao.getList("DAY00713.DAY00713_DS_DUPL_S", map);
					if (duplRecord.size() > 0) {
				 	...
				 	commonDao.insertData("DAY00713.DAY00713_DS_DATA_I", map);
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
				 	commonDao.updateData("DAY00713.DAY00713_DS_DATA_U", map);
				}
			}
		...
		try {
			...
			for (int i = 0; i < rmList.size(); i++) {
				...
				if (rowType == DataSet.ROW_TYPE_DELETED) {
					commonDao.deleteData("DAY00713.DAY00713_DS_DATA_D", map);
				}
			}
