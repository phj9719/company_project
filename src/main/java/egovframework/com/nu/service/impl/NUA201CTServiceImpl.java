...
import egovframework.com.nu.service.NUA201CTService;

@Service("NUA201CTService")
public class NUA201CTServiceImpl implements NUA201CTService {
		...
		try {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED) {
					commonDao.insertData("NUA201CT.NUA201CT_DS_LIST_I", map);
					insertCnt++;
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
					commonDao.updateData("NUA201CT.NUA201CT_DS_LIST_U", map);
					insertCnt++;
				}

//				if (rowType == DataSet.ROW_TYPE_DELETED) {
//					commonDao.updateData("NUA201CT.NUA201CT_DS_LIST_D", map); 
//					
//					String nsSicodeYn = commonDao.getString("CM_BATCH_HEALTH.NS_SICODE_YN_S",map);
//					
//					if(nsSicodeYn.equals("Y")) {
//						commonDao.updateData("CM_BATCH_HEALTH.NS_HEALTH_D", map); 
//					}
//					insertCnt++;
//				}
				if (rowType != DataSet.ROW_TYPE_DELETED) {
					...
					//String nsSicodeYn = commonDao.getString("CM_BATCH_HEALTH.NS_SICODE_YN_S",map);
					String nsSicodeYn = commonDao.getString("CM_BATCH_HEALTH.NU_REF_YN_S",map); // sct_base, sct_mate 존재유무 쿼리
