...
import egovframework.com.nu.service.NUA804QR_DService;

@Service("NUA804QR_DService")
public class NUA804QR_DServiceImpl implements NUA804QR_DService {
	...
	public void select(VariableList inVar, DataSetList inDataset, VariableList outVar,
		...
		log.info("NUA804QR_D select start");
		...
		List<Map<String, Object>> diaperList = commonDao.getList("NUA804QR_D.NUA804QR_D_DS_LIST_S", map);
		log.info("SQLID : NUA804QR_D_DS_LIST_S 실행");
		
		log.debug("NUA804QR_D_DS_LIST_S paging 시작");
		...
		log.debug("NUA804QR_D_DS_LIST_S paging 완료");
		
		outMap.put("output1", diaperPageList);
		
		// 기기 총 개수 등 기타 정보 조회
		List<Map<String, Object>> etcList = commonDao.getList("NUA804QR_D.NUA804QR_D_DS_ETC_LIST_S", map);
		log.info("SQLID : NUA804QR_D_DS_ETC_LIST_S 실행");
		outMap.put("output2", etcList);
		
		log.info("NUA804QR_D select end");
	}
