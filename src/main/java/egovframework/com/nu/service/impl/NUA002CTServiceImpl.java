...
import egovframework.com.nu.service.NUA002CTService;

@Service("NUA002CTService")
public class NUA002CTServiceImpl implements NUA002CTService {
	...
	public int save(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				String newpwd = BCrypt.hashpw(map.get("USER_PASSWD").toString(), BCrypt.gensalt());
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED) {
				 	commonDao.insertData("NUA002CT_DS_LIST_I", map);
				 	insertCnt++;
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
				 	commonDao.updateData("NUA002CT_DS_LIST_U", map);
				}
			}
	...
	public void changePassword(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			String db_pass = commonDao.getString("NUA002CT_PP_PASS_CHK",map);
			...
			if(pass){
				...
				commonDao.updateData("NUA002CT_PP_CHG_PASS",map);
			}else{
	...
	public void clearPassword(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			commonDao.updateData("NUA002CT_PP_CHG_PASS",rowMap);
