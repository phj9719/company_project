...
import egovframework.com.nu.service.NUA202CTService;

@Service("NUA202CTService")
public class NUA202CTServiceImpl implements NUA202CTService {
	...
	public int save(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				String healthYn = commonDao.getString("NUA202CT.NUA202CT_HEALTH_YN",map);
				
				if (rowType == DataSet.ROW_TYPE_INSERTED) {
					commonDao.insertData("NUA202CT.NUA202CT_DS_LIST_I", map);
					insertCnt++;
					if(healthYn.equals("N")) { 
						commonDao.insertData("NUA202CT.NUA202CT_HEALTH_I", map);
					}
					if(healthYn.equals("Y")) { 
						commonDao.updateData("NUA202CT.NUA202CT_HEALTH_U", map);
					}
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
					commonDao.updateData("NUA202CT.NUA202CT_DS_LIST_U", map);
					insertCnt++;
					if(healthYn.equals("N")) { 
						commonDao.insertData("NUA202CT.NUA202CT_HEALTH_I", map);
					}
				...
				//healthData.put("PRE_TIME", map.get("PRE_TIME"));
