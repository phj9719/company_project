...
@Service("NUA101CTService")
public class NUA101CTServiceImpl implements NUA101CTService {
		...
		try {
			...
			for (int x = 0; x < list.size(); x++) {
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED) {
					String newpwd = BCrypt.hashpw(map.get("JU1_PW").toString(), BCrypt.gensalt());
					map.put("APP_PW", newpwd);
				 	commonDao.insertData("NUA101CT_DS_HEAD_I", map);
				 	insertCnt++;
				}
				if (rowType == DataSet.ROW_TYPE_UPDATED) {
				 	commonDao.updateData("NUA101CT_DS_HEAD_U", map);
				}
			}
