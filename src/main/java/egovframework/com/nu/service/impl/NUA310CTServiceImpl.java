...
import egovframework.com.nu.service.NUA310CTService;

@Service("NUA310CTService")
public class NUA310CTServiceImpl implements NUA310CTService {
	...
	public int save(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			DataSet dslist2 = inDataset.get("input2");
			...
			if(list.size()!=0) {
				for (int x = 0; x < list.size(); x++) {
					...
					if (rowType == DataSet.ROW_TYPE_INSERTED) {
						commonDao.insertData("NUA310CT_DS_LIST_I", map);
						insertCnt++;
					}
					if (rowType == DataSet.ROW_TYPE_UPDATED) {
						commonDao.updateData("NUA310CT_DS_LIST_U", map);
					}
					
					String nsHealthYn = commonDao.getString("CM_BATCH_EXCR.NS_EXCR_DATA_YN_S",map);
					String nsSicodeYn = commonDao.getString("CM_BATCH_EXCR.NU_REF_YN_S",map); // sct_base, sct_mate 존재유무 쿼리
					List<Map<String, Object>> nsHomeList = commonDao.getList("CM_BATCH_EXCR.NS_RES_DATA_S",map);
					Map<String, Object> healthData = nsHomeList.get(0);
					if(nsSicodeYn.equals("Y")) {
						if(nsHealthYn.equals("I")) { 
							commonDao.insertData("CM_BATCH_EXCR.NS_EXCR_I", healthData);
						}
						if(nsHealthYn.equals("U")) { 
						 	commonDao.updateData("CM_BATCH_EXCR.NS_EXCR_DATA_U", healthData);
						}
					}
				}
			}
			List<Map<String, Object>> list2 = dataSetUtil.dataSetToList(dslist2);
			if(list2.size()!=0) {
				for (int x = 0; x < list2.size(); x++) {
					...
					if (rowType == DataSet.ROW_TYPE_INSERTED) {
						commonDao.insertData("NUA310CT_DS_LIST2_I", map);
						insertCnt++;
					}
					if (rowType == DataSet.ROW_TYPE_UPDATED) {
						commonDao.updateData("NUA310CT_DS_LIST2_U", map);
					}
					String nsExcrYn = commonDao.getString("CM_BATCH_EXCR.NS_EXCR_DATA_YN_S",map);
					String nsSicodeYn = commonDao.getString("CM_BATCH_EXCR.NU_REF_YN_S",map); // sct_base, sct_mate 존재유무 쿼리
					List<Map<String, Object>> nsHomeList = commonDao.getList("CM_BATCH_EXCR.NS_RES_DATA_S",map);
					Map<String, Object> healthData = nsHomeList.get(0);
					if(nsSicodeYn.equals("Y")) {
						if(nsExcrYn.equals("I")) { 
							commonDao.insertData("CM_BATCH_EXCR.NS_EXCR_I", healthData);
						}
						if(nsExcrYn.equals("U")) { 
						 	commonDao.updateData("CM_BATCH_EXCR.NS_EXCR_DATA2_U", healthData);
						}
					}
				}
			}
		...
		try {
			...
			for (int i = 0; i < rmList.size(); i++) {
				...
				if (rowType == DataSet.ROW_TYPE_DELETED) {
					commonDao.updateData("NUA310CT.NUA310CT_DS_LIST_D", map); 
					
					String nsSicodeYn = commonDao.getString("CM_BATCH_EXCR.NU_REF_YN_S",map);
					
					if(nsSicodeYn.equals("Y")) {
						commonDao.updateData("CM_BATCH_EXCR.NS_HEALTH_D", map); 
					}
					insertCnt++;
				}
			}
