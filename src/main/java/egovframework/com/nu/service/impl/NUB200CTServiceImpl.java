...
@Service("NUB200CTService")
public class NUB200CTServiceImpl implements NUB200CTService {
	...
	public int save(VariableList inVar, DataSetList inDataset, VariableList outVar, Map<String, List<Map<String, Object>>> outMap) throws Exception {
		...
		try {
			...
			String snNo = inVar.getString("SN_NO");
			...
			for (int x = 0; x < list.size(); x++) {
				...
				if (rowType == DataSet.ROW_TYPE_INSERTED || rowType == DataSet.ROW_TYPE_UPDATED) {
					commonDao.insertData("NUB200CT.NUB200CT_DS_HEAD_I", map);
					insertCnt++;
				}
