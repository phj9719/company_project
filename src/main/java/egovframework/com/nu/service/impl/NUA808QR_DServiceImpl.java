...
import egovframework.com.nu.service.NUA808QR_DService;

@Service("NUA808QR_DService")
public class NUA808QR_DServiceImpl implements NUA808QR_DService {
	...
	public void select(VariableList inVar, DataSetList inDataset, VariableList outVar,
		...
		log.info("NUA808QR_D select start");
		String company_cd;	
		...
		List<Map<String, Object>> diaperList = commonDao.getList("NUA808QR_D.NUA808QR_D_DS_LIST_S", map);
		log.info("SQLID : NUA808QR_D_DS_LIST_S 실행");
		outMap.put("output3", diaperList);
		
		log.debug("NUA808QR_D_DS_LIST_S paging 시작");
		...
		log.debug("NUA808QR_D_DS_LIST_S paging 완료");
		
		outMap.put("output1", diaperPageList);
		
		// 기기 총 개수 등 기타 정보 조회
		List<Map<String, Object>> etcList = commonDao.getList("NUA808QR_D.NUA808QR_D_DS_ETC_LIST_S", map);
		log.info("SQLID : NUA808QR_D_DS_ETC_LIST_S 실행");
		outMap.put("output2", etcList);
		
		log.info("NUA808QR_D select end");
	}
